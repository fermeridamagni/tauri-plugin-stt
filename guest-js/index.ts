...
export async function unloadModel(): Promise<void> {
  await invoke("plugin:stt|unload_model");
}
...
export async function onResult(
  ...
  return listenOnBoth<RecognitionResult>("result", "plugin:stt:result", handler);
}

export async function onStateChange(
  handler: (event: StateChangeEvent) => void,
): Promise<UnlistenFn> {
  return listenOnBoth<StateChangeEvent>("stateChange", "plugin:stt:stateChange", handler);
}

export async function onError(handler: (error: SttError) => void): Promise<UnlistenFn> {
  return listenOnBoth<SttError>("error", "plugin:stt:error", handler);
}

async function listenOnBoth<T>(
  pluginEvent: string,
  appEvent: string,
  handler: (payload: T) => void,
): Promise<UnlistenFn> {
  const pluginListener = await addPluginListener<T>("stt", pluginEvent, handler);
  const unlisten = await listen<T>(appEvent, (event) => {
    handler(event.payload);
  });
  return () => {
    unlisten();
